...
<div class="card mt-4 flex min-h-[220px] flex-col items-center justify-center gap-3 p-5 text-center">
  ...
  {#if fase === 'pausa' && step?.tipo === 'responde'}
    ...
    <p class="text-2xl font-extrabold text-terracota">🎤 Parla ora!</p>
    {#key index}
  ...
  {:else if fase === 'aguardando' && step?.tipo === 'responde'}
    ...
    {:else}
      <p class="text-sm text-carvao/50">Detto? Ascolta il madrelingua, oppure vai avanti.</p>
    {/if}
    {#if recUrl}
      <button class="btn bg-oceano text-white text-sm" onclick={ouvirVoce}>▶ ascolta te stesso</button>
    {/if}
  ...
  {#if erroAudio}
    <p role="status" class="text-xs font-medium text-terracota">
      ⚠️ Audio non disponibile — controlla la connessione (o usa "salva offline" prima di partire).
    </p>
  {/if}
</div>
...
<div class="mt-3 flex flex-wrap items-center justify-center gap-4 text-sm">
  <button class="-my-2 px-2 py-3 text-oceano" onclick={repetir}>⟲ ripeti passo</button>
...
{#if micErro}
  <p role="status" class="mt-1 text-center text-xs text-terracota">
    ⚠️ Nessun accesso al microfono — consentilo in Impostazioni iPhone › Bitte? › Microfono.
  </p>
{/if}
...
{#if modo === 'carro'}
  <p class="mt-3 text-center text-xs text-carvao/45">
    🚗 Modalità Auto: scorre da sola dall’inizio alla fine. Usa i pulsanti al volante o la schermata
    di blocco per riprodurre, mettere in pausa e saltare.
  </p>
{/if}

{#if fase === 'fim'}
  <p class="mt-5 text-center text-lg font-semibold text-salvia">✅ Parte completata!</p>
{/if}
