<script lang="ts">
  ...
  async function ouvirFalar(ex: Extract<Exercise, { tipo: 'falar' }>, i: number) {
    ...
    try {
      ...
      falaResultado[i] = { ok: false, ouvido: '(non sono riuscito a sentire — controlla il microfono)' };
    }
  }
    ...
    <div class="card p-4">
      ...
      {#if ex.tipo === 'mcq'}
        ...
        {#if revelado[i]}
          <p class="mt-2 text-sm {escolhas[i] === ex.correta ? 'text-salvia' : 'text-terracota'}">
            {escolhas[i] === ex.correta ? 'Corretto! ✔' : `Era: ${ex.opcoes[ex.correta]}`}
      ...
      {:else if ex.tipo === 'preencher'}
        ...
        <div class="flex flex-wrap items-center gap-2">
          ...
          {#if ex.dica}<span class="text-xs text-carvao/50">suggerimento: {ex.dica}</span>{/if}
        </div>
        {#if revelado[i]}
          <p class="mt-2 text-sm {preencherOk(ex, i) ? 'text-salvia' : 'text-terracota'}">
            {preencherOk(ex, i) ? 'Corretto! ✔' : `Risposta: ${ex.resposta}`}
          </p>
        {/if}
      ...
      {:else if ex.tipo === 'escuta'}
        <div class="mb-3 flex items-center gap-3">
          ...
          <span class="text-sm text-carvao/60">Che cosa hai sentito?</span>
        </div>
        <div class="grid gap-2">
          {#each ex.opcoes as op, o}
            <button
      ...
      {:else if ex.tipo === 'parear'}
        <p class="mb-3 font-medium">Tocca un termine e poi la coppia in italiano.</p>
        ...
        {#if pares[i] && Object.keys(pares[i]).length === ex.pares.length}
          <p class="mt-3 text-sm {ex.pares.every((_, e) => parOk(i, e)) ? 'text-salvia' : 'text-terracota'}">
            {ex.pares.every((_, e) => parOk(i, e))
              ? 'Tutto giusto! ✔'
              : 'Alcune coppie sono scambiate — tocca di nuovo per rifare.'}
          </p>
        {/if}
        ...
        {#if recognitionSupported()}
          ...
          {#if falaResultado[i]}
            <p class="mt-2 text-sm {falaResultado[i]?.ok ? 'text-salvia' : 'text-terracota'}">
              {falaResultado[i]?.ok ? 'Molto bene! ✔' : 'Quasi — riprova.'}
              <span class="text-carvao/60"> (ho sentito: “{falaResultado[i]?.ouvido}”)</span>
            </p>
          {/if}
        {:else}
          <p class="text-sm text-carvao/50">
            Riconoscimento vocale non supportato in questo browser (prova con Chrome sul cellulare).
          </p>
        {/if}
      {/if}
    </div>
