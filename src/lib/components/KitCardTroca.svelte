...
>
  ...
  {#if sua}
    ...
    <button
      ...
      aria-label={revelado ? `Riproduci: ${sua.es}` : 'Rivela la frase'}
    ...
    >
      ...
      {:else}
        ...
        <span class="mt-1 block text-[0.8rem] font-semibold text-oceano"
          >Questa l’hai imparata — tocca per rivelare</span
        >
      {/if}
    </button>
  {/if}
  ...
  {#if falhou}
    <p role="status" class="mt-2 text-[0.8rem] font-semibold text-terracota">
      ⚠️ Nessun audio per questa — scarica il kit prima di volare.
    </p>
  {/if}
</article>
