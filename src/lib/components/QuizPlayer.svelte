...
<div class="card p-4">
  ...
  <div class="mt-3 flex flex-wrap items-center gap-2">
    <button type="button" class="btn-primary" onclick={escutar}>
      {tocando ? '⏸ Pausa' : emMeio || pos > 0 ? '▶ Continua' : '▶ Ascolta la conversazione'}
    </button>
    <button
      type="button"
    ...
    >
      {mostrarTranscricao ? 'Nascondi la trascrizione' : 'Mostra la trascrizione'}
    </button>
  </div>

  {#if erroAudio}
    <p role="status" class="mt-2 text-xs font-medium text-terracota">
      ⚠️ Nessun audio — controlla la connessione e riprova.
    </p>
  {/if}
  ...
  {:else if tocando && idxFala >= 0}
    <p class="mt-3 text-center text-sm text-carvao/50">
      🔊 In riproduzione… ({idxFala + 1}/{quiz.dialogo.length})
    </p>
  {/if}
</div>
  ...
  {#if !corrigido}
    ...
    >
      {tudoRespondido
        ...
        : `Rispondi a tutte (${respostas.filter((r) => r !== null).length}/${quiz.perguntas.length})`}
    </button>
  {:else}
    <div class="card p-5 text-center">
      ...
      <p class="mt-1 text-carvao/70">
        {acertos === quiz.perguntas.length
          ? 'Perfetto. Hai preso tutto. 🎉'
          : acertos >= quiz.perguntas.length - 2
            ? 'Ben fatto. Ci sei quasi. 👏'
            : 'Buon inizio — riascolta e riprova. 💪'}
      </p>
      <div class="mt-4 flex justify-center gap-2">
        <button type="button" class="btn bg-white ring-1 ring-black/10" onclick={refazer}>Riprova</button>
        <button type="button" class="btn-primary" onclick={escutarDoInicio}>Ascolta di nuovo</button>
      </div>
    </div>
  {/if}
</div>
