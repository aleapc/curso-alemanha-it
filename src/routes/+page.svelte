<script lang="ts">
  ...
  function importar() {
    ...
    msgOk = !!r;
    msg = r
      ? `Importato: +${r.ale} tue, +${r.dea} sue. 🎉`
      : 'Codice non valido — controlla di averlo copiato tutto (inizia con ES1.).';
  }
...
<section class="mt-2">
  ...
  <p class="mt-1 text-carvao/70">
    Tedesco di sopravvivenza e cultura per la Germania — ascolta, dillo ad alta voce e non farti fregare.
  </p>
</section>
...
>
  ...
  <span class="min-w-0 flex-1">
    ...
    <span class="block text-xs text-carvao/60"
      >Scarica <b>Google Traduttore</b> e il <b>tedesco</b> per l’uso offline — la fotocamera legge
      menù, etichette e moduli all’istante.</span
    >
  </span>
  <span class="shrink-0 text-lg text-carvao/30">↗</span>
</a>
<p class="mt-1 px-1 text-[11px] text-carvao/45">
  💡 Configuralo PRIMA di partire. Ti salva sulle scritte in piccolo — ma non ordinerà la tua birra, e non
  ti dirà che la stessa bevanda costa tre prezzi diversi.
</p>
...
>
  ...
  <span class="min-w-0 flex-1">
    <span class="block font-bold leading-tight text-carvao">Tasca del viaggiatore</span>
    <span class="block text-xs text-carvao/60"
      >Moneta, tipo di presa e il numero d’emergenza — funziona col telefono in modalità aereo.</span
    >
  </span>
  <span class="shrink-0 text-lg text-carvao/30">→</span>
</a>
  ...
  <section class="mt-7">
    ...
    {#if examDoNivel[nivel.nivel] && nivel.episodios.some((e) => e.partes.some((p) => p.pronta))}
      ...
      >
        ...
        <span class="flex-1">
          <span class="block text-sm font-bold">Prova {nivel.nome}</span>
...
<section class="mt-8">
  <div class="card p-4">
    <h2 class="mb-1 font-bold">👯 Sincronizza col compagno di viaggio</h2>
    <p class="mb-3 text-sm text-carvao/60">
      Genera un codice e invialo per unire quello che avete fatto in due — nessun account, nessun
      server.
    </p>
    ...
    <div class="mt-4 border-t border-black/5 pt-3">
      <p class="mb-2 text-sm text-carvao/60">Hai un codice? Incollalo qui:</p>
      <textarea
        bind:value={cola}
        placeholder="ES1...."
