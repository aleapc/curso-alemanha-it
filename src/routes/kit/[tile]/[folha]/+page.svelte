...
<svelte:head>
  <title>{data.folha.rotulo} · Bitte?</title>
</svelte:head>
    ...
    {:else}
      ...
      <div class="rounded-2xl bg-white p-4 shadow-sm ring-1 ring-black/10">
        ...
        <p class="mt-1 text-[0.9rem] leading-snug text-carvao/75">
          Questa è sulla mappa, ma le frasi per essa non sono ancora arrivate nel kit. Il resto di
          {data.tile.rotulo.toLowerCase()} potrebbe già avere ciò che ti serve.
        </p>
        <a
