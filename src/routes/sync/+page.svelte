<script lang="ts">
  ...
  onMount(() => {
    ...
    if (!data) {
      estado = 'erro';
      detalhe = 'Codice non valido — chiedi di generarne uno nuovo.';
      return;
    }
    const standalone =
...
<div class="mt-10 text-center">
  ...
  {:else if estado === 'preview'}
    ...
    <p class="mt-2 text-carvao/70">
      Questo codice porta {previa?.ale ?? 0} + {previa?.dea ?? 0} parti completate. Importarlo unisce
      tutto e non rimuove nulla.
    </p>
    <button class="btn-primary mt-4" onclick={confirmar}>Importa ora</button>
  {:else if estado === 'ok'}
    <h1 class="text-2xl font-extrabold text-salvia">Fatto ✅</h1>
    <p role="status" class="mt-2 text-carvao/70">{detalhe}</p>
  {:else if estado === 'fora'}
    <h1 class="text-2xl font-extrabold">Ci sei quasi 📲</h1>
    <p class="mx-auto mt-2 max-w-sm text-carvao/70">
      Questo link si è aperto nel tuo <b>browser</b> — ma i tuoi progressi vivono nell’<b>app Bitte?</b> sulla
      tua schermata home. Copia il codice e incollalo lì:
    </p>
    <ol class="mx-auto mt-3 max-w-sm space-y-1 text-left text-sm text-carvao/70">
      <li>1. Tocca <b>Copia codice</b> qui sotto</li>
      <li>2. Apri l’app <b>Bitte?</b> sulla schermata home (su un computer: la home del corso)</li>
      <li>3. In <b>Sincronizza</b>, incollalo e importa</li>
    </ol>
    <button class="btn-primary mt-4" onclick={copiar}>
      {copiado ? '✅ Copiato' : '📋 Copia codice'}
    </button>
    <textarea
